...
# # For testing on k8s
# # def test_pipelines_scenario_scenario_on_k8s(client, add_module_schemas) -> None:  # type: ignore
# #     response = client.post(f"/modules/add", json=add_module_schemas)
# #     assert response.status_code == 200, response.content
# #     obtained_metadata = response.json()
...
# #     infer_schema = {
# #         "target": add_module_schemas["name"],
# #         "inputs": {"input__0": {"data": [0.0] * 3 * 224 * 224}},
# #     }
# #     response = client.post(f"/modules/{add_module_schemas['name']}/infer", json=infer_schema)
# #     assert response.status_code == 200, response.content
# #     result = response.json()
